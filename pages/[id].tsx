    ...
    <div className="flex min-h-screen flex-col">
      <Layout>
        <div className={"flex items-center pb-16 justify-center min-h-[60vh]"}>
          <div
            className={
              "flex lg:mt-32 w-full mt-6 lg:max-w-[1200px] w-full lg:flex-row flex-col lg:justify-center items-center px-4"
            }
          >
            ...
            <div>
              <p className={"text-gray-500 lg:text-xl"}>{data?.description}</p>
              <div>
                <h2 className="lg:text-3xl text-xl py-2 font-semibold">{`₹${data?.price}`}</h2>
                <p className="text-lg text-red-500 font-bold line-through">{`₹${data?.oldPrice}`}</p>
              </div>
              <NumberSelect
                value={quantity}
              ...
              <div className={"flex flex-col w-full  mt-6 space-y-4"}>
                <button className={"w-full py-2 bg-yellow-500 rounded-md"}>
                  Buy now
                </button>
                <button
...
export const getStaticProps: GetStaticProps<{ data: Data }, Params> = async ({
  params,
}) => {
  if (!params?.id) {
    return {
